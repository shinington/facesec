...
def get_pretrained_facenet(mode):
    """
    Return the architecture given a dataset. Note that a nomalize layer is inserted.
    """

    dataset_mean, dataset_std = [0.5, 0.5, 0.5], [128/255., 128/255., 128/255.]
    normalize_layer = get_normalize_layer(dataset_mean, dataset_std)
    resize_layer = get_resize_layer(160)
    if mode == 'closed':
        model = InceptionResnetV1(pretrained='casia-webface', classify=True, num_classes=100)
    else:
        model = InceptionResnetV1(pretrained='casia-webface', classify=False)
    return torch.nn.Sequential(resize_layer.cuda(), normalize_layer, model.cuda()) 
